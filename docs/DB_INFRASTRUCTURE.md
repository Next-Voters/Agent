...
| `description` | `text` | Nullable; longer descriptive blurb, read via `get_region_description()`. |
| `type` | `region_type` (enum) | Currently `country` or `city`. |
| `parent_region` | `text` | Nullable; self-referencing foreign key → `supported_regions.region`. Set on cities to their containing country; `null` for countries. |

Lookup table constraining subscriptions and reports to the vetted list of launch markets. `get_supported_regions_from_db()` groups rows by `type` so the portal can render one region picker per type (e.g. a "Country" dropdown and a "City" dropdown).
...
| `region` | `text` | Nullable; foreign key → `supported_regions.region`. |
...
| `region` | `text` | Foreign key → `supported_regions.region`. |
...
Community-driven region expansion: users vote for regions they want covered. When `vote_count` reaches `threshold`, the request can be approved and the region added to `supported_regions`.
...
```mermaid
erDiagram
    supported_regions ||--o{ subscriptions : "region"
    supported_regions ||--o{ supported_regions : "parent_region"
    supported_regions ||--o{ reports : "region"
...
Key relationships:
- `supported_regions` supplies region references for `subscriptions` and `reports` (one-to-many); each row also carries its own `type` (`country`/`city`) and, for cities, a `parent_region` pointing back at its country.
...
- `get_supported_regions_from_db()` → `dict[str, list[str]]` — queries `supported_regions` (with `type`) and groups region names by type, each list ordered alphabetically
...
- **Region request workflow**: new region requests go into `region_requests` with status `pending`. Votes are recorded in `region_votes`. When `vote_count` meets `threshold`, approve the request and add the region to `supported_regions` (with its `type`).
